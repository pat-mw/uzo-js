...
function checkArtistBookingFromUrl() {
    ...
    if (booking !== null) {
        ...
        artistLinks.each(function () {
            ...
            const artistSlug = artistLink
                .find('[fx-element="artistSlug"]')
                .text();
...
function prepareArtistModalCloseEvents(){
    // search for all elements with the attribute [fx-close-artist-modal]
    const closeElements = document.querySelectorAll('[fx-close-artist-modal]');

    // console.log("close elements: ", closeElements.length);
    // loop through each element and add event listener for click
...
function artistModalClose() {
    // HIDE MODAL - handled by webflow interaction
    console.log("closing artist modal");

    // reset youtube embed and spotify embed to stop sound playing
    let spotifyEmbedEl = $("[artist-modal-spotify-embed]");
    spotifyEmbedEl.attr("src", "");
    spotifyEmbedEl.hide();
    let videoEmbedEl = $("[artist-modal-video-embed]");
    videoIframeContainerEl = $("[artist-modal-video-embed-container]");
    videoEmbedEl.attr("srcdoc", "");
    videoIframeContainerEl.hide();
    let soundcloudEmbedEl = $("[artist-modal-soundcloud-embed]");
    soundcloudEmbedEl.attr("src", "");
...
function addArtist(name, image_url, bio, type, slug) {
    ...
    let radioButton = $(
        'input[name="Type-of-Enquiry"][value="Artist Booking"]'
    );
    radioButton.click();
}
...
function handleFormSectionsDynamic() {
    ...
    $("[fx-element='clearRequestedArtists']").on("click", function () {
        ...
        let radioButton = $(
            'input[name="Type-of-Enquiry"][value="Event Services"]'
        );
        radioButton.click();
        $("[form-first-btn]").hide();
    ...
    $('input[name="Type-of-Enquiry"]').change(function (event) {
        ...
        if (value === "Artist Booking") {
            ...
            if (window.requestedArtists.length > 0) {
                ...
                $('[fx-element="agencyErrorText"]').text(
                    'Error: Cannot select "Event Services" when there are artists in the booking form. Either remove the artists using the link below or select "Both" to proceed.'
                );
                $("[form-first-btn]").show();
                $("[form-first-btn-enabled]").hide();
                return;
...
function handleFormArtistCards() {
    ...
    window.requestedArtists.forEach(function (artist) {
        ...
        template
            .find("[form-artist-template-image]")
...
function prepareAutocompleteForm() {
    ...
    searchItems.find('[fx-element="artistLink"]').on("mousedown", function () {
        console.log(
            "selected artist: ",
            $(this).find('[fx-element="artistName"]').text()
        );
        ...
        const artistImage = $(this)
            .find('[fx-element="artistImage"]')
            .attr("src");
        const artistImageBackup = $(this)
            .find('[fx-element="artistImageBackup"]')
            .attr("src");
        ...
        if (artistImage === "" || artistImage === placeholderUrl) {
            if (
                artistImageBackup === "" ||
                artistImageBackup === placeholderUrl
            ) {
                selectedImage = "";
            } else {
        ...
        addArtist(artistName, selectedImage, "", artistType, artistSlug);
...
// --- HANDLE FORM SCROLL ON STEP CHANGE ---
function handleFormScroll() {
    const formWrapper = document.querySelector(
        '[data-fx-element="contactFormWrapper"]'
    );
    ...
    const scrollForm = function () {
        console.log("clicked button");
        const currentStepTop = $(formWrapper).offset().top;
        console.log("offsetTop:", currentStepTop);
    ...
    nextButtons.forEach(function (button) {
        button.addEventListener("click", scrollForm);
    });
